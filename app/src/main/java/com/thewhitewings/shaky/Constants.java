package com.thewhitewings.shaky;

/**
 * Constants used throughout the app
 */
public class Constants {

    /**
     * Name of the SharedPreferences file
     */
    public static final String PREFERENCES_NAME = "app_preferences";

    /**
     * Key for the preference in the SharedPreferences
     * of whether to display the battery optimization info dialog on app startup
     */
    public static final String DISPLAY_BATTERY_OPTIMIZATION_DIALOG_KEY = "display_battery_dialog";

    /**
     * Key for the sensitivity threshold preference in the SharedPreferences
     */
    public static final String SENSITIVITY_THRESHOLD_KEY = "sensitivity_threshold";

    /**
     * Key for the alarm tone preference in the SharedPreferences
     */
    public static final String ALARM_TONE_KEY = "alarm_tone";

    public static final String NOTIFICATION_INTENT_ACTION_TRAIL = ".ACTION_OPEN_MAIN_FROM_NOTIFICATION";
}